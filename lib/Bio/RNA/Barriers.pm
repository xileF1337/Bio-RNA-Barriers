...
package Bio::RNA::Barriers::Results {
    ...
    sub keep_connected {
        ...
        # # Map the k connected minima to 1..k again.
        # my $i = 1;
        # $_->index($i++)
        #     foreach @connected_mins;                # update min indices
        # $_->father_index($_->father->index)         # update father indices
        #     foreach @connected_mins[1..$#connected_mins]; # min 1 is orphan
...
package Bio::RNA::Barriers::RateMatrix {
    ...
    sub read_text_rate_matrix {
        ...
        ROW: while (defined (my $line = <$input_matrix_fh>)) {
            ...
            confess "Input file contains non-numeric or negative input:\n$line"
                unless all {looks_like_number $_ and $_ >= 0} @row;
    ...
    # Only keep the states connected to the mfe.
    sub keep_connected {
        my ($self) = @_;
...
Return all minima generated by I<Barriers>.
...
the I<Barriers> option C<--saddle>. It can be queried via the C<saddle>
method.
...
C<--saddle> option), it croaks when accessed. Use the C<has_saddle> predicate
to query the status of this attribute.
...
=head3 C<global_saddle_height()>
    ...
    Saddle height as described for saddle_height(), but with respect to the
    lowest neighbor, but to the global mfe structure (basin 1).

=head3 C<more_stuff()>

TODO all the default stuff from barriers file, ...
...
Constructor. Arguments: file_name / file_handle (pass either or both),
file_type (TXT or BIN)
